...
run_dreamer() {
  local seed="$1"
  local run_name="DreamerV3-paper-final-${ROM_NAME}-${PREPROCESS_TAG}-seed${seed}-${RUNNING_STEPS}steps"
  ...
  (
    ...
    "${PYTHON_BIN}" examples/dreamer_v3/dreamer_v3_atari.py \
      ...
      --log-dir "logs/paper-final/dreamer-v3/${PREPROCESS_TAG}/${ROM_NAME}/seed${seed}/" \
      --model-dir "models/paper-final/dreamer-v3/${PREPROCESS_TAG}/${ROM_NAME}/seed${seed}/"
  ) 2>&1 | tee "${log_file}"
}

run_htswm() {
  local seed="$1"
  local run_name="HTS-WM-paper-final-${ROM_NAME}-${PREPROCESS_TAG}-seed${seed}-${RUNNING_STEPS}steps"
  CONFIG_FILE=examples/hierarchical_dreamer/config/atari100k_two_phase.yaml \
  RUN_NAME="${run_name}" \
  ENV_ID="${ENV_ID}" \
  OBS_TYPE="${OBS_TYPE}" \
  NUM_STACK="${NUM_STACK}" \
  ...
  LOG_DIR="logs/paper-final/hts-wm/${PREPROCESS_TAG}/${ROM_NAME}/seed${seed}/" \
  MODEL_DIR="models/paper-final/hts-wm/${PREPROCESS_TAG}/${ROM_NAME}/seed${seed}/" \
...
run_harmony() {
  local seed="$1"
  local run_name="XuanCe-HarmonyDream-paper-final-${ROM_NAME}-${PREPROCESS_TAG}-seed${seed}-${RUNNING_STEPS}steps"
  RUN_NAME="${run_name}" \
  ENV_ID="${ENV_ID}" \
  OBS_TYPE="${OBS_TYPE}" \
  NUM_STACK="${NUM_STACK}" \
  ...
  EVAL_PROTOCOL="${EVAL_PROTOCOL}" \
  PYTHON_BIN="${PYTHON_BIN}" \
  LOG_DIR="logs/paper-final/harmonydream/${PREPROCESS_TAG}/${ROM_NAME}/seed${seed}/" \
  MODEL_DIR="models/paper-final/harmonydream/${PREPROCESS_TAG}/${ROM_NAME}/seed${seed}/" \
  RUN_LOG_DIR="${RUN_LOG_ROOT}" \
...
run_tsae() {
  local seed="$1"
  local run_name="TSAE-style-paper-final-${ROM_NAME}-${PREPROCESS_TAG}-seed${seed}-${RUNNING_STEPS}steps"
  RUN_NAME="${run_name}" \
  ENV_ID="${ENV_ID}" \
  OBS_TYPE="${OBS_TYPE}" \
  NUM_STACK="${NUM_STACK}" \
  ...
  EVAL_PROTOCOL="${EVAL_PROTOCOL}" \
  PYTHON_BIN="${PYTHON_BIN}" \
  LOG_DIR="logs/paper-final/tsae-style/${PREPROCESS_TAG}/${ROM_NAME}/seed${seed}/" \
  MODEL_DIR="models/paper-final/tsae-style/${PREPROCESS_TAG}/${ROM_NAME}/seed${seed}/" \
  RUN_LOG_DIR="${RUN_LOG_ROOT}" \
...
echo "Preprocessing: ${PREPROCESS_TAG}"
